...
const Lobby = () => {
  ...
  const { numOfQuestions, category, difficulty, roundLimit } = useSelector(
    (state) => state.lobby
  );
  const [ModalInvalidLobby, openModalInvalidLobby] = useModal("root", {
    preventScroll: true,
    closeOnOverlayClick: false,
  });
  const [ModalFullLobby, openModalFullLobby] = useModal("root", {
    preventScroll: true,
    closeOnOverlayClick: false,
  });
    ...
    if (isHost === "true") {
      ...
      socket.on("lobby-created", ({ host }) => {
        ...
        setMessages((messages) => [
          `Lobby created by ${host.username}`,
          ...messages,
        ]);
    ...
    <div id="lobby-container">
      ...
      <ModalInvalidLobby>
        <div className="pop-up">
          ...
          <button id="close-pop-up-btn" onClick={() => navigate("/")}>
            Close
          </button>
        </div>
      </ModalInvalidLobby>
      ...
      <div className="players-container">
        ...
        {players.length !== 0 && <ul>{renderPlayers()}</ul>}
      </div>
      {messages.length !== 0 && (
      ...
      {isHost === "false" && !isNewHost && (
        <div className="start-buttons-div">
          ...
          <button onClick={() => leaveLobby(socket)} className="orange-button">
            Leave lobby
          </button>
        </div>
      )}
      {(isHost === "true" || isNewHost) && (
        <div className="start-buttons-div host-buttons">
          <button onClick={startGame} className="green-button">
            Start Game
          </button>
          <button onClick={() => leaveLobby(socket)} className="orange-button">
            Leave lobby
